...
class KotlinLangParser(
    ...
    override fun matches(uri: URI): Boolean =
        uri.host?.endsWith("kotlinlang.org") == true
    ...
    override suspend fun preprocessDocument(
        ...
        val isMarkdown = request.mediaType?.contains("markdown", ignoreCase = true) == true ||
    ...
    override fun discoverLinks(document: JsoupDocument, uri: URI): List<String> =
        document.select("nav a[href], aside a[href]")
            .mapNotNull { node ->
                val resolved = runCatching { uri.resolve(node.attr("href")) }.getOrNull()

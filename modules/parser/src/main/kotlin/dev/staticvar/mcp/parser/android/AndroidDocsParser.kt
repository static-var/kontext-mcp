...
class AndroidDocsParser(
    ...
    override fun matches(uri: URI): Boolean =
        uri.host?.endsWith("developer.android.com") == true
    ...
    override suspend fun preprocessDocument(
        ...
        val contentRoot = document.selectFirst(
        ...
        return if (shell.body().text().isBlank()) {
            val navItems = document.select("div.devsite-book-nav a[href] span.devsite-nav-text, nav.devsite-book-nav a[href] span.devsite-nav-text")
                .map { it.text().trim() }
                .filter { it.isNotEmpty() }
            fallbackLinks = document.select("div.devsite-book-nav a[href], nav.devsite-book-nav a[href]")
                .mapNotNull { anchor ->
            ...
            if (navItems.isNotEmpty()) {
                ...
                fallbackLinks = document.select("a[href]")
                    .mapNotNull { anchor ->
                        val href = anchor.attr("href").takeIf { it.isNotBlank() } ?: return@mapNotNull null
                        val resolved = runCatching { baseUri.resolve(href).normalize().toString() }.getOrNull() ?: return@mapNotNull null
    ...
    override fun determineContentType(uri: URI, document: JsoupDocument): ContentType = when {
    ...
    override fun extractTitle(document: JsoupDocument, uri: URI): String {
    ...
    override fun buildMetadata(document: JsoupDocument, uri: URI): Map<String, String> {
        ...
        val breadcrumb = document.select("nav.breadcrumbs a[href]")
    ...
    override fun discoverLinks(document: JsoupDocument, uri: URI): List<String> {
        ...
        return document.select("nav a[href], aside a[href]")
            .mapNotNull { element ->
                val resolved = runCatching { uri.resolve(element.attr("href")) }.getOrNull()
    ...
    private fun deriveLinksFromRaw(url: String, raw: String): List<String> {
        val baseUri = URI(url)

...
abstract class BaseHtmlParser(
    ...
    final override suspend fun handles(url: String): Boolean = runCatching { URI(url) }.map(::matches).getOrDefault(false)
    ...
    protected open fun extractTitle(
        document: JsoupDocument,
        uri: URI,
    ): String {
    ...
    protected abstract fun determineContentType(
        uri: URI,
        document: JsoupDocument,
    ): ContentType

    protected open fun buildMetadata(
        document: JsoupDocument,
        uri: URI,
    ): Map<String, String> = mapOf("url" to uri.toString())

    protected open fun discoverLinks(
        document: JsoupDocument,
        uri: URI,

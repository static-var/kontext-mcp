        ...
        fun load(config: ApplicationConfig): CrawlerConfig {
            ...
            val session =
                SessionConfig(
                    ...
                    cookieName =
                        sessionConfig.propertyOrNull("cookie-name")?.getString()
                            ?: "mcp_crawler_session",
                    enforceSecureCookie =
                        sessionConfig.propertyOrNull("cookie-secure")?.getString()?.toBooleanStrictOrNull()
                            ?: false,
                    maxAgeSeconds =
                        sessionConfig.propertyOrNull("max-age-seconds")?.getString()?.toLongOrNull()
                            ?: 3 * 60 * 60L,
                )
            ...
            val auth =
                AuthConfig(
                    ...
                    tokenSecret =
                        authConfig.propertyOrNull("token-secret")?.getString()
                            ?: session.secret,
                    tokenTtlSeconds =
                        authConfig.propertyOrNull("token-ttl-seconds")?.getString()?.toLongOrNull()
                            ?: 3600L,
                )

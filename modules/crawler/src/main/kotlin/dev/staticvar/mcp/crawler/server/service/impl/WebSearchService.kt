...
import dev.staticvar.mcp.crawler.server.service.SearchService
...
/**
 * Web-facing search service that coordinates query embedding, vector search, and chunk packing.
 */
class WebSearchService(
    ...
    override suspend fun search(request: SearchRequest): SearchResponse {
        ...
        if (rerankingConfig.enabled && scoredChunks.isNotEmpty()) {
            ...
            scoredChunks =
                rerankedIndices
                    .filter { it in scoredChunks.indices }
                    .map { scoredChunks[it] }
